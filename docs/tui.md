...
| `--yes` / `-y` | `write`/`edit`/`bash`実行前の確認をすべてスキップ(`write`/`edit`はカレントディレクトリ・OS一時ディレクトリ配下ならこのフラグ無しでも既定で自動承認される。下記`/auto`の項参照) |
...
| `/auto` | 確認ゲート(y/N)のON/OFFをトグル。連続してwrite/edit/bashを使うタスクで毎回聞かれたくない時に。確認プロンプト自体で`a`と答えても同様にON(今回だけ承認+以降は自動承認)。**`write`/`edit`はカレントディレクトリまたはOS一時ディレクトリ(`os.tmpdir()`)配下が対象なら、このコマンド無しでも既定で自動承認される**(それ以外のパスは引き続き確認が必要。skillツールはコマンド文字列から対象パスを機械的に特定できないため対象外、常に確認が必要)。**`bash`も、`.core-agent/bash-allowlist.json`に列挙したコマンド(例: `["ls","cat","grep"]`)を単体呼び出し(パイプ・リダイレクト・`&&`/`;`連結を含まない)する場合に限り、同様にこのコマンド無しで自動承認される**(設定ファイルが無ければ何も自動承認されない、既定はオフ。下記「bashの読み取り専用コマンド自動承認」参照) |
| `/reset` | 現在のセッションの会話履歴をクリア |
| `/exit` | 終了 |
| `!<command>` | シェルコマンドを直接実行(LLMを介さない)。`!cd <dir>`で作業ディレクトリ変更 |
| `Esc` | LLM応答のストリーミングを中断(それまでに生成済みのテキストは会話履歴に残る)。実行中のツール(bash等)は対象外 |

`Ctrl-C`は状況によって意味が異なります: `>`プロンプトや確認(y/N)待ちの間に押すと`/exit`と同じ扱いで終了します。
LLM応答待ち・ツール実行中に押しても**何も起きません**(意図的に無視しています。ストリーミング中に途中で
止めたい場合は`Esc`を使ってください)。
...
単一実行ファイル(バイナリ)化については [`docs/binary-build.md`](./binary-build.md) を参照してください。
...
`.core-agent/bash-allowlist.json`に、確認無しで実行してよいコマンド名をJSON配列で列挙できます:

```json
["ls", "cat", "head", "tail", "grep", "pwd"]
```

- 対象は「シェルメタ文字(`; & | < > `` ` `` `$`・改行)を一切含まない単体呼び出し」のみ。
  パイプ・リダイレクト・`&&`/`;`連結が絡む時点で通常の確認に戻る
- 設定ファイルが無ければ何も自動承認されない(既定はオフ、`hooks.json`/`cron.json`と同じ
  「作らなければ何も起きない」opt-in方式)——**どのコマンドを安全とみなすかはユーザーが
  決めるポリシーであり、coreは特定のコマンド名を一切ハードコードしていない**
- `write`/`edit`の自動承認(上記`/auto`の項参照)と違い、パスがカレントディレクトリ/
  一時ディレクトリ配下かどうかは問わない(`read`/`search`/`list`ツールが元々パス無制限で
  確認ゲート無しなのと同じ信頼水準に揃えているため)
- `find`(`-exec`/`-delete`等の副作用フラグを持ちうる)やgitサブコマンドは、単純な
  コマンド名一致だけでは安全性を判定しきれないため、自分でリストに追加しないことを推奨
...
- `command`はskillディレクトリを作業ディレクトリとして実行され、ツール呼び出しの引数がJSONとして
  **標準入力**に渡される。標準出力がそのままツール結果になる(非ゼロ終了は失敗として扱われる)。
  任意の言語で実装可能(Python等)。
- skillツールも`write`/`edit`/`bash`と同じ確認ゲート(`ctx.confirm`)を通る。ただし
  `write`/`edit`のカレントディレクトリ・OS一時ディレクトリ配下への自動承認(上記`/auto`の項参照)は
  skillツールには適用されない(skillコマンドは任意のシェルコマンドであり単一の対象パスを
  機械的に特定できないため)——skillツールは常に確認が必要。
...
### 同梱skill: `pdf_export`(Markdown→PDF)

MarkdownテキストをPDFに変換する`export_pdf`ツールを同梱しています(`skills/pdf_export`)。
インストール済みのGoogle Chromeでレンダリングするため(`CORE_AGENT_CHROME`でパス変更可)、
日本語/CJKテキストもシステムフォントでそのまま正しく出力されます(Python製PDFライブラリで
よくあるフォント抽出・登録の手間が不要)。見出し・箇条書き・テーブル・コードブロック・
太字/斜体・リンクに対応しています。
...
- プロセスを起動したままにする必要がある(`node-cron`の内部タイマーがイベントループを維持)。
  停止はCtrl-C。
- **GUI版(`docs/gui.md`)も同じ`.core-agent/cron.json`を共有します**。TUI・GUIどちらか
  一方を起動しっぱなしにしていればcronは動きます。**両方同時に起動していると同じジョブが
  二重登録され、同じ通知が複数回発火することがあるので注意してください。**
...
- **日本語IME入力時の表示崩れ**: 変換候補がターミナルの右端で折り返すと表示が乱れる場合が
  ...
  Node標準の`readline`ベースの実装では根本的な解決が難しい既知の問題です。確定済みの
  テキストを貼り付ける分には問題ありません。**この問題が無いGUI版**([`docs/gui.md`](./gui.md))も
  用意しています。
...
- **Windows実機でのテストは実施済み**: `bash`ツール・Chrome自動検出はWindows向けの実装
  (OS既定シェルの自動選択、`taskkill`によるプロセスツリー終了、標準的なインストール先パス)を
  入れており、実機テストで見つかった問題(V8 bytecodeキャッシュ非互換によるクラッシュ、
  `cmd.exe`でのpip installコマンドのクォート崩れ、ネットワーク瞬断時のLLMリクエスト失敗で
  プロセス全体が落ちる問題、`visit_page`経由でのbot検知回避の抜け)はいずれも修正・
  実機再検証済みです。ただし全機能を網羅した検証ではないため、未発見の問題が残っている
  可能性はあります。
