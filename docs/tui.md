...
| `/auto` | 確認ゲート(y/N)のON/OFFをトグル。連続してwrite/edit/bashを使うタスクで毎回聞かれたくない時に。確認プロンプト自体で`a`と答えても同様にON(今回だけ承認+以降は自動承認)。**`write`/`edit`はカレントディレクトリまたはOS一時ディレクトリ(`os.tmpdir()`)配下が対象なら、このコマンド無しでも既定で自動承認される**(それ以外のパスは引き続き確認が必要。`bash`・skillツールはコマンド文字列から対象パスを機械的に特定できないため対象外、常に確認が必要) |
